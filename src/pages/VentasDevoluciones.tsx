// src/pages/VentasDevoluciones.tsx
// ============================================================================
// Devoluciones de venta — entrada de mercadería devuelta por clientes.
//
...
// (IN). El ajuste económico es un documento separado (Notas de crédito).
//
...
//   · TPMovementLinesEditor (direction="IN")
...
type SalesReturn = {
  id: string;
  number: string;           // "DVV-0001"
  date: string;             // ISO
  client: string;
  sourceDocument: string;   // "FV-0001" / "REM-0001" — opcional
...
function returnedItemsCount(r: SalesReturn): number {
...
export default function VentasDevoluciones() {
  ...
  const [draft, setDraft]           = useState<SalesReturn | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return returns.filter((r) => {
      ...
      if (!term) return true;
      return `${r.number} ${r.client} ${r.sourceDocument}`.toLowerCase().includes(term);
    });
  }, [returns, q, statusFilter, clientFilter]);

  // ── Opciones de filtros ──────────────────────────────────────────────────
  const clientOptions = useMemo(() => {
    const uniq = Array.from(new Set(returns.map((r) => r.client).filter(Boolean))).sort();
    return [
  ...
  // ── Crear / editar ────────────────────────────────────────────────────────
  function openNew() {
    const blank: SalesReturn = {
      id:             uid(),
      number:         nextDocNumber("DVV", returns),
      date:           todayISO(),
      client:         "",
      sourceDocument: "",
      warehouse:      "",
  ...
  function openEdit(r: SalesReturn) {
  ...
  function confirmReturn(r: SalesReturn) {
    // Placeholder: en Fase 6 generará ArticleMovement IN (sourceType=SALES_RETURN)
  ...
  return (
    <TPSectionShell
      title="Devoluciones"
      subtitle="Entrada de mercadería devuelta por clientes"
      right={
    ...
    >
      <div className="space-y-4">
        ...
        <TPTableKit<SalesReturn>
          rows={filtered}
          columns={COLS}
          storageKey="tp_sales_returns_cols"
          search={{
            value: q,
            onChange: setQ,
            placeholder: "Buscar por número, cliente o documento origen…",
            debounceMs: 150,
          }}
          sortPersistKey="tp_sales_returns"
          columnPicker
          headerLeft={filters}
...
function EditorModal(props: {
  ...
  draft: SalesReturn;
  isNew: boolean;
  onChange: (next: SalesReturn) => void;
  onSave: () => void;
  onClose: () => void;
}) {
  const { open, draft, isNew, onChange, onSave, onClose } = props;

  function patch<K extends keyof SalesReturn>(key: K, value: SalesReturn[K]) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isNew ? "Nueva devolución de venta" : `Editar devolución ${draft.number}`}
      subtitle={`Número ${draft.number}`}
      maxWidth="3xl"
      resizable
      maximizable
      maximizedMode="embedded"
      modalKey="ventas-devoluciones-editor"
      onEnter={onSave}
      footer={
      ...
      <div className="space-y-4">
        <TPCard title="Datos principales">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <TPField label="Documento origen" hint="Ej: FV-0001 / REM-0001 (opcional)">
              <TPInput
        ...
        >
          {draft.lines.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border px-4 py-8 text-center text-sm text-muted">
              Sin líneas. Agregá artículos para registrar qué está devolviendo el cliente.
            </div>
          ) : (
