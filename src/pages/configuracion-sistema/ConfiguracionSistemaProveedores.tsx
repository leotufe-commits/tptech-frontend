...
import { ENTITY_COLS, SUPPLIER_COL_LS_KEY } from "./clientes/clientes.constants";
...
export default function ConfiguracionSistemaProveedores() {
  ...
  async function load() {
    setLoading(true);
    try {
      const result = await commercialEntitiesApi.list({ role: "supplier", take: 200 });
      setRows(result.rows);
    } catch (e: any) {
  ...
  return (
    ...
    >
      <EntidadesTable
        ...
        sortDir={sortDir}
        onSort={toggleSort}
        storageKey={SUPPLIER_COL_LS_KEY}
        columns={ENTITY_COLS}
        onEdit={openEdit}
      ...
      <ConfirmDeleteDialog
        ...
        description="¿Estás seguro? Solo podés eliminar proveedores sin movimientos en cuenta corriente."
