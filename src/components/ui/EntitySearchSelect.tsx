// src/components/ui/EntitySearchSelect.tsx
// Selector de proveedor/cliente con búsqueda asíncrona.
// Visual idéntico a ArticleSearchSelect / TPComboCreatable.
...
export default function EntitySearchSelect({
  ...
  onClear: () => void;
  /** Filtra por tipo: "client", "supplier" o "all" (por defecto) */
  role?: "client" | "supplier" | "all";
  placeholder?: string;
}) {
  const defaultPlaceholder =
    role === "client"   ? "Buscar cliente…"
    : role === "supplier" ? "Buscar proveedor…"
  ...
  const [options,     setOptions]     = useState<EntityRow[]>([]);
  ...
  // ── Resetea índice activo al cambiar opciones ────────────────────────────
  useEffect(() => {
  ...
  function search(val: string) {
    ...
    timer.current = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await commercialEntitiesApi.list({ role, q: val, take: 10 });
        setOptions(res.rows);
      } finally {
        setLoading(false);
      }
  ...
  function openDropdown() {
    if (selected) {
      ...
      timer.current = setTimeout(async () => {
        setLoading(true);
        try {
          const res = await commercialEntitiesApi.list({ role, q: "", take: 10 });
          setOptions(res.rows);
        } finally {
          setLoading(false);
        }
  ...
  const inputValue = selected ? selected.displayName : query;
